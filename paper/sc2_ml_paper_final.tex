...
\hypersetup{colorlinks=true,linkcolor=blue!50!black,citecolor=blue!50!black,urlcolor=blue!60!black,pdftitle={StarCraft II Match Outcome Prediction: From Replay Parsing to Group-Aware Experimental Validation},pdfauthor={Jose}}
...
\fancyhead[R]{Pineda Fritas Jose Luis}
...
\begin{document}
	...
	\begin{titlepage}
		...
		\begin{tcolorbox}[width=0.88\textwidth,colback=softblue,colframe=accent,arc=2mm,boxrule=0.8pt]
			This document is designed as the \textbf{main technical paper} of the project. It consolidates the project methodology, explains the logic of the pipeline, and reports the final replay-aware experimental results preserved in the repository for reproducibility review.
		\end{tcolorbox}
		\vfill
		\begin{tabular}{rl}
			Candidate: & Pineda Fritas Jose Luis \\
			Academic year: & 2025--2026 \\
			Document version: & Final revised version \\
			Date: & March 2026 \\
		\end{tabular}
		\vfill
	\end{titlepage}
	...
	A central methodological result is that naive row-based validation substantially overestimates performance because adjacent snapshots from the same replay are highly correlated. Once the split is performed by \texttt{replay\_id}, the final evaluation settles near 61\%, which is far more realistic and scientifically meaningful. Tree-based models then emerge as the strongest and most stable baseline on this tabular representation. The final GPU-trained MLP remains competitive but does not surpass the tree ensembles, and the strongest remaining advantage of XGBoost over Random Forest appears in probabilistic quality rather than in a large gain in raw accuracy. Overall, the results support replay-aware tree ensembles as the strongest and most reliable baseline for this tabular representation of StarCraft II match state.
	...
	\begin{resultbox}
		Replay-aware validation settles near \textbf{61\%}. This should be treated as the credible estimate for the preserved benchmark, whereas naive row-based validation is methodologically invalid because it leaks information across snapshots from the same replay.
	\end{resultbox}
	
	\section{Model Families and Expected Behavior}
	\subsection{Random Forest}
	Random Forest is a strong baseline for this dataset because the inputs are already engineered tabular variables with nonlinear thresholds and interactions \cite{breiman2001rf}. It does not require feature scaling, is robust to heterogeneous feature magnitudes, and can learn discrete rule-like partitions that fit many RTS situations.
	
	\subsection{XGBoost}
	XGBoost is attractive for the same reasons as Random Forest, but with sequential residual correction and stronger capacity to exploit subtle interactions \cite{chen2016xgboost}. The current implementation uses GPU acceleration with histogram-based tree construction and early stopping on a dedicated validation split.
	
	\subsection{MLP}
	The MLP branch is conceptually interesting because it tests whether a deep feedforward network can extract smooth nonlinear structure from the tabular representation, using a GPU-trained PyTorch implementation in the final branch \cite{paszke2019pytorch}. The frozen final branch uses standardization, BatchNorm, dropout, ReLU activations, AdamW optimization, and validation-log-loss candidate selection under a replay-aware split. However, the deep model is also the most sensitive to methodological mistakes in validation and scaling.
...
\end{itemize}
The aligned Random Forest branch keeps replay-aware splitting and GroupKFold model selection on the training partition, while a separate export-preserved RF run is retained in the repository to match the historical Test 1--4 artifacts.
...
\begin{itemize}
    ...
    \item an XGBoost pipeline with proper outer split, inner split, grouped model selection, and early stopping,
    \item a Random Forest pipeline with grouped model selection,
    \item a GPU-aware PyTorch MLP pipeline using replay-aware train/validation/test splitting and validation-based candidate selection.
\end{itemize}

\clearpage

\section{Appendix B: Supplementary Note on the POMDP-like Interpretation}
A separate supplementary note expands the conceptual formulation introduced near the end of the main paper. Its purpose is to formalize the distinction between latent strategic state, replay-derived observation, high-level strategic action, observation quality, and belief-like estimation from historical replay trajectories.
...
\begin{thebibliography}{9}
	...
	\newblock In \emph{Proceedings of the 22nd ACM SIGKDD International Conference on Knowledge Discovery and Data Mining (KDD)}, pages 785--794, 2016.
	...
	Adam Paszke, Sam Gross, Francisco Massa, Adam Lerer, James Bradbury, Gregory Chanan, Trevor Killeen, Zeming Lin, Natalia Gimelshein, Luca Antiga, Alban Desmaison, Andreas K\"opf, Edward Yang, Zachary DeVito, Martin Raison, Alykhan Tejani, Sasank Chilamkurthy, Benoit Steiner, Lu Fang, Junjie Bai, and Soumith Chintala.
	\newblock PyTorch: An Imperative Style, High-Performance Deep Learning Library.
	\newblock In \emph{Advances in Neural Information Processing Systems 32 (NeurIPS)}, pages 8024--8035, 2019.
